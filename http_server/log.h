...
class Log{
  ...
  private:
    Log(const std::string& n,const std::string& p=".",int mut_thread = false,int l=0);
    Log(const Log&);
    Log operator=(const Log&);
    ~Log();
  public:
    static Log& create(const std::string& n,const std::string& p=".",int mut_thread = false,int l =0);
    bool need_open_new_file();
    int open_new_file();
    void release_file();
    void update_next_time();
    int print(int l,const char* file_name,int line,const char* func,const char* fmt,...);
    void lock();
    void unlock();
  private:
