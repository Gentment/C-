...
Log::Log(const std::string& n,const std::string& p,int mult_thread,int l)
...
int Log::print(int l,const char* file_name,int line,const char* func,const char* fmt,...){
  (void)l;
  if(level != Log::ALL && level < 1){
    return 1;
  }
  lock();
...
void Log::get_hour_min_sec(std::string& res){
  time_t cur_time = time(NULL);
  struct tm *tieminfo = localtime(&cur_time);

  char buffer[] = {"hh:mm:ss"};
  strftime(buffer,sizeof(buffer),"%T",tieminfo);
  res = std::string(buffer);
}
